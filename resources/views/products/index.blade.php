          ...
          <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <a href="{{ route('products.show', $product) }}" class="bg-amber-300 hover:bg-amber-500 py-1.5 px-3 rounded mr-2">Lihat</a>
                      <a href="{{ route('products.edit', $product) }}" class="text-white bg-blue-600 hover:bg-blue-800 py-1.5 px-3 rounded mr-2">Edit</a>
                      <form action="{{ route('products.destroy', $product) }}" method="POST" class="inline">
                          @csrf
                          @method('DELETE')
                          <button type="submit" class="text-white bg-red-500 hover:bg-red-700 py-1.5 px-3 rounded" onclick="return confirm('Yakin hapus?')">Hapus</button>
                      </form>
                  </td>
              </tr>
              @endforeach
